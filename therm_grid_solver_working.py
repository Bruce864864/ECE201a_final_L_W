...
def box_color(box):
    c = getattr(box, "chiplet_parent", None)
    t = str(c.get_chiplet_type()) if c is not None else ""
    if "GPU" == t:
        return "orange"
    if t.startswith("HBM"):
        return "blue"
    if t in ["substrate", "PCB", "interposer"]:
        return "gray"
    if "Power_Source" in t or "Power_Source" in box.name:
        return "purple"
    if "TIM" in box.name:
        return "green"
    if "bonding" in box.name:
        return "brown"
...
def draw_fig(boxes,out_dir,out_name,limits):
    ...
    legend_items = [
        ...
        Patch(facecolor="gray", label="Substrate/Interposer/PCB"),
        Patch(facecolor="purple", label="Power Source"),
    ...
    boxes_sorted = sorted(
        boxes,
        key=lambda b: (float(b.start_z), float(b.width*b.length)),
    )
    for box in boxes_sorted:
        c = box_color(box)
        z = 3 if c in ["orange", "blue"] else 1
    ...
    #for box in boxes:
        #ax.add_patch(plt.Rectangle((box.start_x, box.start_y), box.width, box.length, fill=True, color=box_color(box)))
    ...
    plt.tight_layout()
    plt.savefig(out_dir+'/'+out_name+'.png', dpi=100, bbox_inches="tight")
    plt.close()

def determine_draw_lim_3d(boxes):
    min_x = min(b.start_x for b in boxes)
    max_x = max(b.start_x + b.width for b in boxes)
    min_y = min(b.start_y for b in boxes)
    max_y = max(b.start_y + b.length for b in boxes)
    min_z = min(b.start_z for b in boxes)
    max_z = max(b.start_z + b.height for b in boxes)
    pad = 5
...
def simulator_simulate_lumped(
    ...
    def ez(b): return b.start_z + b.height
    def ex(b): return b.start_x + b.width
    def ey(b): return b.start_y + b.length
    ...
    def contact_area_xy_mm2(b1, b2):
        ox = overlap_1d(b1.start_x, ex(b1), b2.start_x, ex(b2))
        oy = overlap_1d(b1.start_y, ey(b1), b2.start_y, ey(b2))
        return ox * oy

    def extract_material_name(stackup_str):
        if stackup_str is None:
            return None
        s = str(stackup_str)
        if ":" not in s:
            return s.strip()
    ...
    def box_k_W_mK(b):
        m = extract_material_name(getattr(b, "stackup", None))
        if m in conductivity_values:
            return conductivity_values[m]
        if m is None:
            return conductivity_values.get("Si", 105)
        ml = m.lower()
        if "tim" in ml:
            return conductivity_values.get("TIM", 100)
        if "si" in ml:
            return conductivity_values.get("Si", 105)
        if "cu" in ml:
            return conductivity_values.get("Cu-Foil", 400)
        if "air" in ml:
            return conductivity_values.get("Air", 0.025)
        if "epoxy" in ml or "epag" in ml:
            return conductivity_values.get("Epoxy, Silver filled", 1.6)
        return conductivity_values.get("Si", 105)
    ...
    if isinstance(heatsink_obj, dict) and "hc" in heatsink_obj:
        hc_try = to_float(heatsink_obj["hc"], None)
        if hc_try is not None:
            hc = hc_try
    if hc < 500:
        hc *= 1000.0
    ...
    for i, bx in enumerate(all_boxes):
        P = 0.0
        c = getattr(bx, "chiplet_parent", None)
        if c is not None:
            t = str(c.get_chiplet_type())
            if t == "GPU":
                P = 400.0
            elif t == "HBM":
                P = 5.0
    ...
    for i in range(n):
        bi = all_boxes[i]
        ki = box_k_W_mK(bi)
        hi_m = max(float(bi.height), 1e-9) * 1e-3
        for j in range(i + 1, n):
            bj = all_boxes[j]
            bj_ez = ez(bj)
            bi_ez = ez(bi)
            gap1 = abs((bi.start_z + bi.height) - bj.start_z)
            gap2 = abs((bj.start_z + bj.height) - bi.start_z)
            touch = (gap1 < 0.05) or (gap2 < 0.05)
            if not touch:
                continue
            area_mm2 = contact_area_xy_mm2(bi, bj)
            if area_mm2 <= 0.0:
                continue
            area_m2 = area_mm2 * 1e-6
            kj = box_k_W_mK(bj)
            hj_m = max(float(bj.height), 1e-9) * 1e-3
            R = (0.5 * hi_m) / (ki * area_m2) + (0.5 * hj_m) / (kj * area_m2)
            G = 1.0 / max(R, 1e-15)
            A[i, i] += G
            A[j, j] += G
            A[i, j] -= G
    ...
    tim_idx = []
    for i, bx in enumerate(all_boxes):
        name_str = str(bx.name)
        mat_str = str(getattr(bx, "stackup", ""))
        if ("_TIM" in name_str) or ("TIM" in name_str) or ("TIM" in mat_str):
            tim_idx.append(i)

    if len(tim_idx) == 0:
        z_top = max(ez(bx) for bx in all_boxes)
        tim_idx = [i for i, bx in enumerate(all_boxes) if abs(ez(bx) - z_top) < 1e-3]
    ...
    for bx in boxes:
        ...
        if c is None:
            continue
        t = str(c.get_chiplet_type())
        if (t == "GPU") or (t.startswith("HBM")):
            top_boxes.append(bx)

    if len(top_boxes) == 0:
        z_top = max((b.start_z + b.height) for b in all_boxes)
        top_idx = [i for i,b in enumerate(all_boxes) if abs((b.start_z + b.height) - z_top) < 1e-3]
    else:
        top_names = set(b.name for b in top_boxes)
        top_idx = [i for i,b in enumerate(all_boxes) if b.name in top_names]

    for i in top_idx:
        bx = all_boxes[i]
        area_mm2 = max(float(bx.width) * float(bx.length), 1e-12)
        area_m2 = area_mm2 * 1e-6
        G = max(hc * area_m2, 1e-12)
        A[i, i] += G
        bvec[i] += G * T_amb
    ...
    for i, bx in enumerate(all_boxes):
        ...
        avg = float(T[i])
        k = box_k_W_mK(bx)
        area_m2 = max(float(bx.width) * float(bx.length), 1e-12) * 1e-6
        hz_m = max(float(bx.height), 1e-9) * 1e-3
        Rz = hz_m / (k * area_m2)
        results[bx.name] = (peak, avg, 0.0, 0.0, float(Rz))
    return results
...
def box_priority(box):
    ...
    if t == "GPU":
        return 60
    if t in ["substrate", "interposer", "PCB"]:
        return 50
    if "Power_Source" in t or "Power_Source" in name:
        return 40
    return 0    

def point_in_box(x, y, z, box, eps=1e-9):
    return (
        box.start_x - eps <= x <= box.start_x + box.width + eps and
        box.start_y - eps <= y <= box.start_y + box.length + eps and
        box.start_z - eps <= z <= box.start_z + box.height + eps
    )
    
def effective_k_for_box(box):
...
def build_cut_planes(all_boxes):
    xs, ys, zs = [], [], []
    for b in all_boxes:
        xs.extend([b.start_x, b.start_x + b.width])
        ys.extend([b.start_y, b.start_y + b.length])
...
def filter_tiny_cells(cells, min_dx=1e-3, min_dy=1e-3, min_dz=1e-4):
    out = []
    for c in cells:
        if c["dx"] < min_dx:
            continue
...
def assign_cells_to_boxes(cells, all_boxes):
    for cell in cells:
        hits = []
        for b in all_boxes:
            if point_in_box(cell["cx"], cell["cy"], cell["cz"], b):
                hits.append(b)

        if not hits:
            continue

        hits.sort(key=lambda b: (box_priority(b), -(b.width * b.length * b.height)), reverse=True)
        owner = hits[0]
        cell["owner"] = owner
        cell["k"] = effective_k_for_box(owner)

    solid_cells = [c for c in cells if c["owner"] is not None]
    return solid_cells
    
def get_hbm_stack_key(box):
    name = box.name
    parts = name.split(".")
    for p in parts:
        if p.startswith("HBM#"):
...
def assign_power_to_cells(cells):
    gpu_cells = []
    hbm_groups = {}
    ...
    for c in cells:
        ...
        if b is None:
            continue
        if getattr(b, "chiplet_parent", None) is None:
            continue

        t = b.chiplet_parent.get_chiplet_type()

        if t == "GPU":
            gpu_cells.append(c)

        key = get_hbm_stack_key(b)
        if key is not None:
            hbm_groups.setdefault(key, []).append(c)

    if gpu_cells:
        total_vol = sum(c["dx"] * c["dy"] * c["dz"] for c in gpu_cells)
        if total_vol > 0:
            for c in gpu_cells:
                frac = (c["dx"] * c["dy"] * c["dz"]) / total_vol
                c["power"] += 400.0 * frac

    for key, group in hbm_groups.items():
        if not group:
            continue
...
def solve_grid_cells(cells, xs, ys, zs, hc=15000.0, T_amb=25.0):
    n = len(cells)
    idx_map = {(c["ix"], c["iy"], c["iz"]): i for i, c in enumerate(cells)}

    A = np.zeros((n, n), dtype=float)
    b = np.zeros((n,), dtype=float)

    def add_conduction(i, j, direction):
        ci, cj = cells[i], cells[j]
        ki, kj = ci["k"], cj["k"]

        if direction == "x":
            area_mm2 = ci["dy"] * ci["dz"]
            Ri = (0.5 * ci["dx"] * 1e-3) / (ki * area_mm2 * 1e-6)
            Rj = (0.5 * cj["dx"] * 1e-3) / (kj * area_mm2 * 1e-6)
        elif direction == "y":
            area_mm2 = ci["dx"] * ci["dz"]
            Ri = (0.5 * ci["dy"] * 1e-3) / (ki * area_mm2 * 1e-6)
            Rj = (0.5 * cj["dy"] * 1e-3) / (kj * area_mm2 * 1e-6)
        else:
            area_mm2 = ci["dx"] * ci["dy"]
            Ri = (0.5 * ci["dz"] * 1e-3) / (ki * area_mm2 * 1e-6)
            Rj = (0.5 * cj["dz"] * 1e-3) / (kj * area_mm2 * 1e-6)

        G = 1.0 / max(Ri + Rj, 1e-18)
        A[i, i] += G
        A[j, j] += G
        A[i, j] -= G
        A[j, i] -= G
...
def collect_results_from_cells(cells, T):
    from collections import defaultdict
    groups = defaultdict(list)

    for c, temp in zip(cells, T):
        groups[c["owner"].name].append((c, temp))

    results = {}
    for name, vals in groups.items():
        temps = [v[1] for v in vals]
        peak = max(temps)
        ...
        for c, _ in vals:
            ...
            ay = dx_m * dz_m
            az = dx_m * dy_m
            rx_list.append(dx_m / (k * ax))
            ry_list.append(dy_m / (k * ay))
            rz_list.append(dz_m / (k * az))

        results[name] = (
            float(peak),
            float(avg),
            float(sum(rx_list) / len(rx_list)),
            float(sum(ry_list) / len(ry_list)),
            float(sum(rz_list) / len(rz_list)),
        )
